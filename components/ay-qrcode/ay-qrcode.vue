<template>
	<view :class="modal?'show-qrcode':'hide-qrcode'">
		...
		<view class="box-qrcode"  @longtap="longtapCode">
			...
			<canvas class="canvas-qrcode" :style="style_w_h" style="display: flex;height: 200px;
    width: 200px;
...
;" :canvas-id="qrcode_id">
				...
				<view v-if="modal&&is_themeImg" :style="style_w_h" class="box-img-qrcode">
					<image :style="style_w_h_img"  :src="themeImg"></image>
				</view>
				<!-- #endif -->
...
<script>
	...
	export default {
		...
		computed: {
			...
			style_w_h_img() {
				let that = this;
				var height = parseInt(that.h_w_img+10);
		...
		methods: {
			...
			longtapCode(){
				var that = this;
				console.log('太好听好听',that.qrcode_id)
				//#ifndef MP
				uni.showModal({
					title: '校正二维码',
					content: '二维码是否异常',
