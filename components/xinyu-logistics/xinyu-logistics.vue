<template>
	<view class="page">
		...
		<view class="express">
			...
			<view class="right">
				<!-- <view class="text">{{wlInfo.post_name}} {{wlInfo.logistic_code}}</view> -->
				<view class="text">{{'物流单号：'}} {{wlInfo.logistic_code}}</view>
			</view>
		</view>
		...
		<view class="content bgf">
			<view>
				...
				<view class="flex list" :class="{one: index == 0 && wlInfo.state == 3}" v-for="(item, index) in wlInfo.traces.slice().reverse()"
				 :key="index">
					<view class="time">
					...
					<view class="info flex1">
						<!-- {{index == 0 && wlInfo.state == 3 ? '已签收' : '配送中'}} -->
						<view class="title" v-if="index == 0 && wlInfo.state == 3">
							{{ '已签收' }}
						</view>
						<view class="title" v-if="index > 0 && item.Action == 202">
							{{ '派件中' }}
						</view>
						<view class="title" v-if="index > 0 && item.Action == 201">
							{{ '到达派件城市' }}
						</view>
						<view class="title" v-if="index > 0 && item.Action == 1">
							{{ '已揽件' }}
						</view>
						<view class="title" v-if="index > 0 && item.Action == 2">
							{{ '在途中' }}
						</view>
