	...
	<view class="hostShops">
		<view class="search-with" :style="[{'paddingTop':statusBar+'px'}]">
			...
			<view class="search-input">
				<uni-search-bar placeholder="输入搜索内容" bgColor="#fff"  @confirm="search" :focus="false" v-model="search_keyword" radius="100" cancelButton="none" @blur="blur" @input="input" >
				</uni-search-bar>
			</view>

		</view>
		<view class="ht-banner">
			<image :src=storeDeitals.url mode="aspectFill"></image>
		</view>
		<view class="spBG">
			<view class="flexs spMess">
					<!-- <view class="spImg" @click="orderChange(1)" :class="orstatus==1?'orderCol':''">
						综合
					</view> -->
					<view class="spImg" @click="orderChange(2)" :class="orstatus==2?'orderCol':''">
						销量
					</view>

					<view class="spImg" @click="orderChange(3)" :class="orstatus==3?'orderCol':''">
						价格
					</view>
	...
	export default {
		...
		onLoad(options) {
			...
			if(options.active && options.active !=undefined)
			{
				...
				this.activeBoot=options.active
			}
		...
		methods: {
			orderChange(index)
			{
				...
				this.sort==''
				this.order_by=''
				if(this.sort=='asc' && index==2)
				{
					this.sort='desc'
					this.order_by='sales'
				}else{
					if(this.sort=='desc' && index==2)
					{
						this.sort='asc'
						this.order_by='sales'
					}
				}
				if(this.sort=='asc' && index==3)
				{
					this.sort='desc'
					this.order_by='price_in_cents'
				}else{
					if(this.sort=='desc' &&  index==3)
					{
						this.sort='asc'
						this.order_by='price_in_cents'
					}
				...
				this.diffSearch(this.activeBoot)
			...
			IndexBanner()
			{
				this.api.interactive.IndexBanner().then(res=>{
					this.storeDeitals=res.data.slideshow
[0]
				})
			},
