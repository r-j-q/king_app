<template>
	<view class="orderitals">
		...
		<!-- <block v-for="(item,index) in odeitals.orders" :key="item.id"> -->
		<block v-for="(item,index) in odeitals" :key="item.id">
			<view class="Swr8FcdU3ZIrrhasu9RL">
				<image :src=item.store.business_license_file_url mode=""></image>
				<view class="shopNems">{{item.store.name}}</view>


			</view>
			<block v-for="(its,ins) in item.order_products" :key="its.id">
			...
			<view class="shopMess">
				<view class="flexs setpan">
					...
					<view class="">
						{{odeitals.orders[0].serial_number}}
					</view>
				</view>
				<view class="flexs setpan">
					<view class="">
					...
					<view class="">
						{{getCurrentTime(odeitals.orders[0].order_products[0].created_at)}}
					</view>
				</view>
				<view class="flexs setpan">
					<view class="">
				...
				<view class="flexs setpan">
					...
					<view class="">
						{{odeitals.orders[0].order_comment?odeitals.orders[0].order_comment:'无'}}
					</view>
				</view>


				<view class="gopay" v-if="odeitals.orders[0].status==0">
					<button @click="subPay">确认付款</button>
				</view>
			</view>
		...
		<view class="wlShow">
			<view class="wlmess">
				<view @click="wlChange">物流信息</view>
				<!-- <image v-if="wlShow" src="/static/img/xx.png" mode=""></image>
				<image v-else src="/static/img/xs.png" mode=""></image> -->
			</view>
	...
	export default {
		...
		onLoad(options) {
			if(options.orderId)
			{
				...
				this.newGetorderdeit(options.orderId)
			}
		},
		methods:{
			wlChange()
			...
			{
				uni.navigateTo({
					url:'/pages/payOrder/payOrder?orderId='+this.odeitals.out_trade_no
				})
			},
			getCurrentTime(t) {
