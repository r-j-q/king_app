<template>
	<view class="orderWith">
			...
			<view class="setbomm" v-if="orderList.length >0">
				<view class="order-status" @click.stop="orderDeitals(item)"  v-for="(item,index) in orderList" :key="item.id">
					...
					<view class="flexs setor">
						<view class="stopWith" @click.stop="goStore(item.store.id)">
							<view class="stlogo">
								<image :src=item.store.logo_file_url></image>
							</view>

							<view class="stName cartFl">
								<view class="">
									{{item.store.name}}
								</view>
								<view>
									<uni-icons type="right" color="#666" size="16"></uni-icons></view>
								</view>
						</view>
					...
					<view class="orMess-img" v-for="(sitem,sindex) in item.order_products" :key="sitem.id">
						...
						<view class="payorder" v-if="item.status==4">
							<view class="ordate">
								<text>共计{{sitem.count}}件商品</text>,
								<text>单价金额:￥  {{sitem.price_in_cents/100}}</text>
								<text v-if="sitem.marketing_rule==2" style="padding: 10upx;">+{{sitem.required_redemption_point*sitem.count}} 代金券</text>
					...
					<block v-if="item.status==0">
						...
						<view class="payorder" style="margin-top: 30upx;">

							<button @click.stop="subOrder(item)">确认付款</button>
						</view>
					</block>
					...
					<block v-if="item.status==2">
						...
						<view class="payorder wMesssub" style="margin-top: 30upx;">
							<view class="iswu">
								<button @click.stop="seleWu(item)">查看物流</button>
							</view>
							<view class="">
								<button @click.stop="subConfirmgoods(item)">确认收货</button>
							</view>
		...
		<!-- 代付款-->
		<view v-if="tabs==2">
			<view class="setbomm" v-if="unlist.length >0">
				<view class=" order-status" @click.stop="orderDeitals(item)" v-for="(item,index) in unlist" :key="item.id">
					<view class="flexs setor">
						<view class="stopWith" @click="goStore(item.orders[0].store.id)">
							<view class="stlogo">
								<image :src=item.orders[0].store.logo_file_url></image>
							</view>
							<view class="stName cartFl">
								<view class="">
						...
						<view class="orderType" v-if="item.orders[0].status==0">待付款</view>
						<view class="orderType" v-if="item.status==1">待发货</view>
						<view class="orderType" v-if="item.status==2">待收货</view>
						<view class="orderType" v-if="item.status==3">已完成</view>
						<view class="orderType" v-if="item.status==4">已取消</view>
					...
					<view class="orMess-img" v-for="(sitem,sindex) in item.orders[0].order_products" :key="sitem.id">
						<view class="flexs">
							<view class="orImg">
								<image :src=sitem.product_cover_image_url></image>
								<view class="ortype">
						...
						<view class="payorder" v-if="item.orders[0].status==0">
					...
					<block v-if="item.orders[0].status==0">
						<view class="allmp" style="font-size: 28upx;color: #D02C14;font-weight: 600;text-align: end;">
							合计金额: ￥ {{item.orders[0].product_price_in_cents/100}}
							<block v-if="item.orders[0].used_redemption_point >0">
								{{item.orders[0].used_redemption_point}} + 代金券
							</block>
						</view>
						<view class="payorder" style="margin-top: 30upx;">
		...
		<view v-if="tabs==5 ">
			<view class="setbomm" v-if="orderList.length >0 ">
				<view class=" order-status" @click.stop="orderDeitals(item)" v-for="(item,index) in orderList"
				 :key="item.id">
	...
	export default {
		...
		methods: {
			...
			Seleorder(pams)
			{
				...
				this.api.interactive.Seleorder(ispams).then(res=>{
			...
			getCurrentTime(t) {
			 ...
			 seleWu(item)
			 {
				 ...
				 uni.navigateTo({
				 	...
				 	url:'/pages/orderList/deitals?orderId='+item.serial_number
				 })
			 ...
			 orderDeitals(item)
			 {
				uni.navigateTo({
				 ...
				 console.log('订单详情serial_number',item)
			 }
