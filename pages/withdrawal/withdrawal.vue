<template>
	<view class="wdran">
		<view class="flexs iotops" :style="[{'paddingTop':statusBar+'px'}]">
			...
			<view class="sl" :class="tabs==1?'active':'noict'">提现</view>
		...
		<view class="isMoney">
			...
			<view class="canMoney sfmg">
				<text>手续费：￥ {{sf}}</text>
				<text class="twoleft">代扣税：￥ {{df}}</text>
			</view>
		</view>
		...
		<view class="withMethods">
			...
			<view class="perMssban">
				<view class="perFlex">
					<view class="bName">*姓名</view>
					<view class="perint">
						<input type="text" @input="onKeyInput"  maxlength="5" v-model="form.user_name"  placeholder-style="color:#C0C4CC;font-size:28upx;" placeholder="请输入姓名,最多输入5个字符">
					</view>
				</view>
			</view>
			<view class="perMssban">
				<view class="perFlex">
					<view class="bName">*银行</view>
					<view class="perint" @click.stop="plBank"  style="color:#C0C4CC;font-size:28upx;">
						<!-- <view class="">请选择银行</view> -->
						<view class="setbakd">
							<view>
								<input type="text"   disabled="disabled" v-model="form.bank_name"  placeholder-style="color:#C0C4CC;font-size:28upx;" placeholder="请选择银行卡">
							</view>
							<view>
								<uni-icons type="right" color="#666" size="22"></uni-icons>
							</view>
			...
			<view class="perMssban">
				<view class="perFlex">
					<view class="bName">*身份证号</view>
					<view class="perint">
						<input type="idcard"  maxlength="20"  v-model="form.id_card" @blur="isbankCard"  placeholder-style="color:#C0C4CC;font-size:28upx;" placeholder="请输入身份证号,尾号X注意大写">
					</view>
				</view>
			</view>
			<view class="perMssban">
			...
			<view class="explain">
				...
				<view class="w1">
					 应税务流程要求，提现到账7个工作日(节假日顺延)
				</view>
				<view class="w1">
					 <text>提现人年龄必须</text>
					 <text style="color: #FF0E0E;">满16周岁且不超过60周岁，不满足此条件的用户将无法提现</text>
				</view>
			</view>		
		</view>
				...
				<template v-if="poptype==2">
					...
					<view class="perMssban">
						<view class="perFlex">
							<view class="bName">*姓名</view>
							<view class="perint">
								<input type="text"   maxlength="5"  v-model="form1.user_name"  placeholder-style="color:#C0C4CC;font-size:28upx;" placeholder="请输入姓名,最多输入5个字符">
							</view>
						</view>
					</view>
					<view class="perMssban">
						<view class="perFlex seleOpt">
							<view class="bName">*银行卡</view>
							<view class="seopsty">
								<uni-data-select
					...
					<view class="perMssban">
						<view class="perFlex">
							<view class="bName">*身份证号</view>
							<view class="perint">
								<input type="idcard"  maxlength="20"  v-model="form1.id_card" @blur="isbankCard"  placeholder-style="color:#C0C4CC;font-size:28upx;" placeholder="请输入身份证号,尾号X注意大写">
							</view>
						</view>
					</view>
					<view class="perMssban">
		...
		<uni-popup ref="widthpopup">
			<view class="taskTips">
				<view class="tipcont">
					<text>本月提现金额已超10万，请认证企业账号后</text>
...
<script>
	...
	export default {
		...
		methods: {
			...
			laschange(pams)
			{
				console.log('怕没啥事',pams)
				if(pams==2)
				{
					this.$refs.widthpopup.close()
				}else{
					...
					this.api.user.Getauthentication({
						...
						console.log('res',res)
						if(res.data.state==1 && res.code==200)
						{
							uni.navigateTo({
			...
			IntInput(e) {
				...
				this.sf=parseFloat(this.widata.withdrawConfig/100*val).toFixed(2)  
				this.df=parseFloat(this.widata.taxConfig/100*val).toFixed(2)
				  if(val >0 && val)
				  {
					 this.dc=parseInt(val)-(parseInt(this.widata.withdrawConfig/100*val)+parseInt(this.widata.taxConfig/100*val)) 
				  }else{
					  this.dc=0
				  }
			...
			goPayaccount()
			{
				...
				this.api.interactive.userApplication(form).then(res=>{
					...
					if(err.code==423)
					{
						this.$refs.widthpopup.open('center')
					}else{
						uni.showToast({
							title:err.message,
							icon:'none',
